...
public class MyListingsGUI extends BaseGUI {
    ...
    public MyListingsGUI(NDUltimateShop plugin, Player player) {
    ...
    @Override
    public void open() {
        this.listings = new ArrayList<>(plugin.getAuctionManager().getPlayerListings(player.getUniqueId()));
        sortItems(listings);

        String titleKey = plugin.getLanguageManager().getMessage("gui-mylistings-title");
        String title = titleKey.replace("%count%", String.valueOf(listings.size()));
    ...
    private void setupGUI() {
        ...
        if (slots.containsKey("close")) {
            ...
            if (meta != null) {
                meta.setDisplayName(plugin.getLanguageManager().getMessage("gui-mylistings-back"));
                backButton.setItemMeta(meta);
            }
            inventory.setItem(slots.get("close"), backButton);
        }

        if (slots.containsKey("my-listings")) {
            int myListingSlot = slots.get("my-listings");
            // Kiểm tra nếu slot này chưa có item (để tránh ghi đè nếu config trùng slot)
            if (inventory.getItem(myListingSlot) == null || inventory.getItem(myListingSlot).getType() == Material.AIR) {
                inventory.setItem(myListingSlot, filler);
            }
        }
    ...
    private void processDecorativePlaceholders(ItemStack item) {
        ...
        ph.put("total_listings", String.valueOf(plugin.getAuctionManager().getAllListings().size()));
        ph.put("player_listings", String.valueOf(listings.size()));
        ph.put("commission", String.valueOf((int) plugin.getAuctionManager().getCommissionFee()));
        ph.put("balance", String.format("%,.2f", plugin.getEconomy().getBalance(player)));
    ...
    private ItemStack createAuctionDisplay(AuctionListing listing) {
        ...
        ph.put("action_text", config.getMessage("action-cancel"));
